...
class SimplifiedP2PService:
    ...
    def __init__(self, config: P2PConfig, agent: "PraxisAgent"):
        ...
        logger.debug(f"Supported protocols: A2A, CARD, TOOL, EXCHANGE")
        logger.debug(f"Persistent connections enabled: True")
    ...
    async def _handle_card_stream(self, stream: INetStream):
        ...
        try:
                ...
                if peer_skills:
                    ...
                    for skill in peer_skills:
                        if isinstance(skill, dict):
                            skill_names.append(skill.get('name', skill.get('id', 'unknown')))
                        else:
                            skill_names.append(str(skill))
                    logger.info(f"🔧 Peer skills: {skill_names} ({len(peer_skills)} total)")
                else:
                    logger.info(f"🔧 Peer skills: [] (0 total)")
                logger.info(f"⚡ Peer capabilities: streaming={peer_capabilities.get('streaming', False)}, pushNotifications={peer_capabilities.get('pushNotifications', False)}")
                if peer_transports:
                    logger.info(f"🌐 Peer transports: {peer_transports}")
                if peer_provider.get('name'):
                    logger.info(f"🏢 Peer provider: {peer_provider.get('name')} v{peer_provider.get('version', 'unknown')}")
            ...
            if my_skills:
                ...
                logger.info(f"🔧 Our skills: [] (0 total)")
            logger.info(f"⚡ Our capabilities: streaming={my_capabilities.get('streaming', False)}, pushNotifications={my_capabilities.get('pushNotifications', False)}")
            if my_transports:
                logger.info(f"🌐 Our transports: {my_transports}")
            if my_provider.get('name'):
                logger.info(f"🏢 Our provider: {my_provider.get('name')} v{my_provider.get('version', 'unknown')}")
            
            await stream.write(json.dumps(my_card).encode("utf-8"))
            
            # Final success message
            logger.success(f"🔄 Card exchange completed with {peer_name} ({peer_id[:8]}...) (responder)")
            
            # Publish event
            await self._publish_event(
                EventType.P2P_CARD_EXCHANGE,
                {"peer_id": peer_id, "card": peer_card}
            )
    ...
    async def _perform_full_exchange(self, stream: INetStream, peer_id: str):
        ...
        try:
            ...
            if peer_message.get("type") == "card_exchange":
                ...
                logger.error(f"❌ Invalid card exchange message from {peer_id}: expected 'card_exchange', got '{peer_message.get('type')}'")
                return
            ...
            peer_name = peer_card.get('name', 'unknown')
            logger.success(f"🔄 Card exchange completed with {peer_name} ({peer_id[:8]}...)")
            logger.info(f"✅ Exchange summary: {peer_name} has {len(peer_tools)} tools and {len(peer_card.get('skills', []))} skills")
                ...
                with trio.move_on_after(30) as cancel_scope:  # 30 second timeout
                    ...
                    if data:
                        ...
                        if response.get("type") == "tool_request":
                            ...
                            await self._handle_persistent_tool_request(stream, response, peer_id)
    ...
    async def _exchange_cards_initiator(self, stream: INetStream):
        ...
        try:
            logger.info(f"🔄 Starting card exchange with {peer_id} (card-only initiator)")
            ...
            try:
                ...
                # Final success message
                logger.success(f"🔄 Card exchange completed with {peer_name} ({peer_id[:8]}...) (card-only initiator)")
    ...
    async def _initiate_persistent_exchange(self, peer_id: PeerID) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to initiate persistent exchange with {peer_id_str}: {e}")
    ...
    async def _perform_full_exchange_initiator(self, stream: INetStream, peer_id: str):
        ...
        try:
            ...
            peer_card = self.peer_cards.get(peer_id, {})
            peer_name = peer_card.get('name', 'unknown')
            logger.success(f"🔄 Card exchange completed with {peer_name} ({peer_id[:8]}...) (initiator)")
            logger.info(f"✅ Exchange summary: {peer_name} has {len(peer_tools)} tools and {len(peer_card.get('skills', []))} skills")
    ...
    async def _invoke_tool_persistent(self, peer_id_str: str, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            with trio.move_on_after(60) as cancel_scope:  # 60 second timeout
                while True:
                    ...
                    if data:
                        response = json.loads(data.decode('utf-8'))
                        if (response.get("type") == "tool_response" and 
    ...
    async def _connect_bootstrap_nodes(self):
        ...
        for i, bootstrap_addr in enumerate(self.config.bootstrap_nodes):
            ...
            try:
                logger.info(f"Connecting to bootstrap node {i+1}/{len(self.config.bootstrap_nodes)}: {addr_str}")
                ...
                if result.get("status") == "connected":
                    ...
                    logger.success(f"Successfully connected to bootstrap node: {result.get('peer_id')}")
        ...
        logger.info(f"Bootstrap connection summary: {successful_connections}/{len(self.config.bootstrap_nodes)} successful")
    ...
    async def call_remote_tool_by_peer_name(self, peer_name: str, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def get_peer_status_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "ready_connections": len([p for p, s in self.connection_states.items() if s == ConnectionState.READY]),
            "peers": []
        }
        ...
        for peer_id in self.connected_peers:
            peer_info = {
                ...
                "state": self.connection_states.get(peer_id, ConnectionState.DISCONNECTED),
...
def create_simplified_p2p_service(config: P2PConfig, agent: "PraxisAgent") -> SimplifiedP2PService:
