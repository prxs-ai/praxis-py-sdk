...
class MDNSDiscovery:
    ...
    def __init__(self, service_name: str, port: int):
        ...
        self._listener: MDNSServiceListener | None = None
    ...
    async def start(self):
        ...
        try:
            ...
            self._browser = AsyncServiceBrowser(
                self._azc.zeroconf, self.service_type, self._listener
            )
    ...
    async def stop(self):
        ...
        try:
            ...
            if self._service_info and self._azc:
                await trio_asyncio.aio_as_trio(self._azc.async_unregister_service)(
                    self._service_info
                )
    ...
    async def announce_service(self, peer_id: str, addresses: list[str]):
        ...
        try:
            ...
            await trio_asyncio.aio_as_trio(self._azc.async_register_service)(
                self._service_info
            )

            logger.info(
                f"Announced mDNS service for peer {peer_id} on {local_ip}:{self.port}"
            )
    ...
    async def _on_service_change(self, action: str, service_data: dict[str, Any]):
        ...
        try:
            ...
            if action == "add" or action == "update":
                ...
                logger.info(
                    f"Discovered peer via mDNS: {peer_id} at {peer_info['addresses']}"
                )
...
class P2PDiscovery:
    ...
    def __init__(self, host: IHost, event_bus: EventBus, config: Any):
        ...
        self._running = False
        self._discovery_task: trio.CancelledError | None = None

        # mDNS service
        self.mdns = MDNSDiscovery(
            service_name=self.service_tag, port=getattr(config, "port", 9000)
        )
    ...
    async def start(self):
        ...
        try:
            ...
            await self.mdns.announce_service(
                str(self.host.get_id()), [str(addr) for addr in self.host.get_addrs()]
            )
    ...
    async def _connect_to_peer(self, peer_info: PeerInfo):
        ...
        current_time = time.time()
        if (
            peer_info.connection_attempts >= self.max_connection_attempts
            and current_time - peer_info.last_connection_attempt < 300
        ):  # 5 minutes
            return
                ...
                if is_connected != peer_info.is_connected:
                    ...
                    if is_connected:
                        ...
                        await self.event_bus.publish(
                            "p2p.peer_connected", peer_info.to_dict()
                        )
                    else:
                        logger.info(f"Peer {peer_id} disconnected")
                        await self.event_bus.publish(
                            "p2p.peer_disconnected", peer_info.to_dict()
                        )
    ...
    async def _exchange_cards_with_peer(self, peer_id: str):
        ...
        try:
            ...
            await self.event_bus.publish(
                "p2p.request_card", {"peer_id": peer_id, "auto_exchange": True}
            )
    ...
    async def get_peer_info(self, peer_id: str) -> dict[str, Any] | None:
    ...
    async def force_connect_to_peer(self, peer_id: str, addresses: list[str]):
        ...
        peer_info = PeerInfo(
            id=peer_id, addresses=addresses, found_at=time.time(), last_seen=time.time()
        )
