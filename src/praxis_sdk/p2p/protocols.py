...
class ProtocolRegistry:
    ...
    def get_handler(self, protocol_id: str) -> Optional[Callable]:
...
class P2PProtocolHandler:
    ...
    def __init__(self, host: IHost, event_bus: EventBus, config: Any):
        ...
        self.our_card: Optional[AgentCard] = None
    ...
    async def handle_card_stream(self, stream: INetStream):
        ...
        try:
            ...
            if data:
                ...
                # Publish event
                await self.event_bus.publish("p2p.card_exchanged", {
                    "peer_id": peer_id,
                    "card": peer_card.model_dump()
                })
                
                logger.info(f"✅ Card exchange complete: {peer_card.name} v{peer_card.version}")
    ...
    async def handle_tool_stream(self, stream: INetStream):
        ...
        try:
            ...
            logger.info(f"📥 Tool request: {request.name} with args: {request.arguments}")
        ...
        try:
            while True:
                ...
                try:
                    ...
                    logger.debug(f"A2A request: {rpc_request.method} ID: {rpc_request.id}")
    ...
    async def _execute_tool_request(self, request: ToolRequest, peer_id: str) -> ToolResponse:
    ...
    async def _process_mcp_message(self, message: P2PMessage, peer_id: str) -> P2PMessage:
        ...
        try:
            ...
            if message.method == "initialize":
                ...
                resources = resources_data.get("resources", []) if resources_data else []
    ...
    async def _process_a2a_request(self, request: JSONRPCRequest, peer_id: str) -> JSONRPCResponse:
    ...
    # Public API methods
    async def request_card(self, peer_id: str) -> Optional[AgentCard]:
        ...
        try:
            ...
            try:
                ...
                if data:
                    ...
                    logger.info(f"✅ Received card: {peer_card.name} v{peer_card.version}")
                    return peer_card
    ...
    async def send_mcp_request(self, peer_id: str, request: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def send_tool_response(self, peer_id: str, response_data: Dict[str, Any]) -> bool:
        """Send tool execution response to a peer"""
        try:
            response = ToolResponse(
                id=response_data.get("request_id", "unknown"),
                result=response_data.get("result") if response_data.get("success") else None,
    ...
    async def invoke_tool(self, peer_id: str, tool_name: str, arguments: Dict[str, Any]) -> Any:
    ...
    async def send_a2a_request(self, peer_id: str, request: JSONRPCRequest) -> JSONRPCResponse:
        ...
        try:
            ...
            try:
                ...
                if data:
                    ...
                    logger.info(f"✅ A2A request completed")
                    return response
