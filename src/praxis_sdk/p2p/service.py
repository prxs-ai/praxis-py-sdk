...
class P2PService:
    ...
    def stop(self):
        ...
        if self.host and self._trio_token:
            try:
                trio.from_thread.run_sync(self._close_host, trio_token=self._trio_token)
            except Exception as e:
    ...
    def _get_security_options(self) -> dict:
        ...
        return {PLAINTEXT_PROTOCOL_ID: InsecureTransport(local_key_pair=self.keypair)}
            ...
            for bootstrap_addr in self.config.bootstrap_nodes:
                ...
                try:
                    ...
                    if "/p2p/" not in addr_str:
                        ...
                        try:
                            ...
                            if host_label:
                                ...
                                keystore_root = str(
                                    Path(self.config.keystore_path).parent
                                )
                                seed_path = os.path.join(
                                    keystore_root, host_label, "node.key"
                                )
                                pid = self._compute_peer_id_from_seed(seed_path)
                                if pid:
                                    addr_str = addr_str.rstrip("/") + f"/p2p/{pid}"
                                    logger.info(
                                        f"Resolved bootstrap addr via keystore -> {addr_str}"
                                    )
                        except Exception as e:
                            logger.debug(
                                f"Could not resolve peer id for {addr_str}: {e}"
                            )
                    ...
                    if cancel_scope.cancelled_caught:
                        logger.warning(
                            f"Bootstrap connection to {peer_info.peer_id} timed out"
                        )
                        continue
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Failed to exchange cards with bootstrap node: {e}"
                        )
