...
class P2PServiceFixed:
    ...
    async def _handle_card_stream(self, stream: "INetStream"):
        ...
        try:
            ...
            logger.info(f"Card exchanged with {peer_id}: {peer_card.get('name', 'unknown')}")
    ...
    async def _connect_bootstrap_nodes(self):
        ...
        for bootstrap_addr_str in self.config.bootstrap_nodes:
            try:
                logger.info(f"Attempting to connect to bootstrap node: {bootstrap_addr_str}")
                ...
                if "/p2p/" not in bootstrap_addr_str:
                    ...
                    resolved_addr = await self._resolve_bootstrap_peer_id(bootstrap_addr_str)
                    if resolved_addr:
                        bootstrap_addr_str = resolved_addr
                    else:
                        logger.warning(f"Could not resolve peer ID for {bootstrap_addr_str}, skipping")
                        continue
                ...
                try:
                    ...
                    logger.warning(f"Failed to exchange cards with {peer_info.peer_id}: {e}")
                
            except Exception as e:
                logger.error(f"Failed to connect to bootstrap node {bootstrap_addr_str}: {e}")
            
            # Small delay between connections
            await trio.sleep(1)
    
    async def _resolve_bootstrap_peer_id(self, addr_str: str) -> Optional[str]:
        ...
        try:
            ...
            if parts[0] == "ip4" and parts[2] == "tcp":
                ...
                keystore_candidates = [
                    ...
                    f"./keys/agent1/node.key" if port == "4001" else None,
                    f"./keys/agent2/node.key" if port == "4002" else None,
                    f"./keys/orchestrator/node.key",
                    f"./keys/worker-filesystem/node.key"
                ]
    ...
    async def _exchange_cards_initiator(self, stream: "INetStream"):
        ...
        try:
            ...
            logger.info(f"Initiated card exchange with {peer_id}: {peer_card.get('name', 'unknown')}")
