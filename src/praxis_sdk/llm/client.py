...
class LLMClient:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(f"LLM init: provider=openai, model={self.config.model}, base_url={self.config.base_url or 'https://api.openai.com/v1'}, api_key_set={bool(self.config.api_key)}, timeout={timeout_val}, retries={self.config.max_retries}")
            ...
            try:
                ...
                if tools:
                    for t in tools[:5]:
                        if isinstance(t, dict):
                            fn = t.get('function', {}).get('name') if 'function' in t else t.get('name')
                            if fn:
                                first_names.append(fn)
                logger.info(f"🧠 LLM request: model={self.config.model}, messages={len(messages)}, tools={tool_count}, tool_choice={tool_choice}")
                if first_names:
                ...
                if messages:
                    ...
                    if last_user:
                        ...
                        logger.debug(f"   📨 User: {content[:120]}{'...' if len(content)>120 else ''}")
            except Exception:
                pass
                ...
                if hasattr(msg, 'tool_calls') and msg.tool_calls:
                    ...
                    for i, tc in enumerate(msg.tool_calls, 1):
                        try:
                            ...
                            logger.info(f"   [{i}] {name} args={args[:200] if isinstance(args, str) else args}")
                        except Exception:
                            pass
                else:
