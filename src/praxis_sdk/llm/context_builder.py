...
class AgentCapability:
    ...
    execution_time_avg: float | None = None
...
class AgentInfo:
    ...
    def is_available(self) -> bool:
        ...
        return (
            ...
            and (datetime.now() - self.last_seen).seconds < 300  # Seen within 5 minutes
        )
...
class NetworkContextBuilder:
    ...
    def __init__(self, p2p_service: P2PService, mcp_registry: ToolRegistry):
        ...
        self._metrics_cache: NetworkMetrics | None = None
    ...
    async def build_network_context(
        self, include_performance_history: bool = True, max_age_seconds: int = 300
    ) -> NetworkContext:
        ...
        try:
            ...
            agents, tools, metrics = await asyncio.gather(
            ...
            logger.success(
                f"Built network context in {build_time:.2f}s: {metrics.active_agents} agents, {len(tools)} tools"
            )
    ...
    async def get_agent_recommendations(
        self, tool_name: str, requirements: dict[str, Any] | None = None
    ) -> list[dict[str, Any]]:
                ...
                if tool_name in agent_data.get("capabilities", []):
                    ...
                    if agent_info and agent_info.is_available:
                        ...
                        score = self._calculate_agent_suitability(
                            agent_info, tool_name, requirements
                        )
    ...
    async def get_network_health(self) -> dict[str, Any]:
        ...
        try:
            ...
            agent_health = min(
                metrics.active_agents / max(metrics.total_agents, 1), 1.0
            )
            connectivity_health = metrics.connectivity_score
            performance_health = max(
                0.0, 1.0 - (metrics.average_response_time / 5000.0)
            )  # 5s = 0 health

            overall_health = (
                agent_health + connectivity_health + performance_health
            ) / 3.0
            ...
            return {"status": "error", "reason": str(e), "overall_health": 0.0}
                    ...
                    for tool_name, tool_info in agent_card.capabilities.items():
                        capabilities.append(
                            AgentCapability(
                                ...
                                execution_time_avg=self._get_avg_execution_time(
                                    peer_id, tool_name
                                ),
                                success_rate=self._get_tool_success_rate(
                                    peer_id, tool_name
                                ),
                            )
                        )
            ...
            for server_name, tool_list in discovered_tools.items():
                for tool in tool_list:
                    ...
                    tool_info = ToolInfo(
                        ...
                        average_execution_time=self._get_tool_avg_execution_time(
                            tool_name
                        ),
    ...
    async def _calculate_network_metrics(self) -> NetworkMetrics:
        ...
        try:
            ...
            active_agents = sum(
                1 for agent in self._agent_cache.values() if agent.is_available
            )
            ...
            if self._agent_cache:
                avg_response_time = sum(
                    agent.response_time_avg for agent in self._agent_cache.values()
            ...
            if active_agents == 0:
                ...
                avg_load = sum(
                    agent.load_percentage for agent in self._agent_cache.values()
                ) / len(self._agent_cache)
                if avg_load < 30:
                    network_load = "low"
                elif avg_load < 70:
    ...
    def _build_context_from_cache(self) -> NetworkContext:
        ...
        return NetworkContext(
            available_agents=self._format_agents_for_context(
                list(self._agent_cache.values())
            ),
            available_tools=self._format_tools_for_context(
                list(self._tool_cache.values())
            ),
            network_load=self._metrics_cache.network_load
            if self._metrics_cache
            else "unknown",
            agent_capabilities=self._build_capability_map(
                list(self._agent_cache.values())
            ),
    ...
    def _update_caches(
        self, agents: list[AgentInfo], tools: list[ToolInfo], metrics: NetworkMetrics
    ):
    ...
    def _format_agents_for_context(
    ...
    def _calculate_agent_suitability(
        self, agent_info: AgentInfo, tool_name: str, requirements: dict[str, Any] | None
    ) -> float:
        ...
        if requirements:
            if (
                "max_load" in requirements
                and agent_info.load_percentage > requirements["max_load"]
            ):
                score *= 0.5
    ...
    def _get_health_recommendations(
        self, health_score: float, metrics: NetworkMetrics
    ) -> list[str]:
        ...
        if metrics.average_response_time > 3000:
            recommendations.append(
                "High response times detected - check network connectivity"
            )
