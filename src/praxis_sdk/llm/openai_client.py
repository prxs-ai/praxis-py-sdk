...
from .prompts import WorkflowPrompts, ErrorRecoveryPrompts, format_prompt
...
class WorkflowPlannerClient:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.success(f"Workflow planner client initialized with model: {self.model}")
    ...
    async def generate_workflow_from_natural_language(
        ...
        try:
            ...
            context = {
                ...
                "available_agents": self._format_agents(network_context.available_agents),
            ...
            # Format prompt
            formatted_prompt = format_prompt(WorkflowPrompts.WORKFLOW_GENERATION, context)
            ...
            self.stats["average_confidence"] = (
                self.stats["average_confidence"] * (self.stats["plans_generated"] - 1) + plan.confidence
            ) / self.stats["plans_generated"]
            
            logger.success(f"Generated workflow plan {plan_id} in {processing_time:.2f}s")
            return plan
            ...
            if max_retries > 0:
                logger.info(f"Retrying workflow generation ({max_retries} attempts left)")
    ...
    async def analyze_execution_error(
        ...
        try:
            ...
            formatted_prompt = format_prompt(ErrorRecoveryPrompts.ERROR_ANALYSIS, context)
    ...
    def _find_agent_with_tool(self, tool_name: str, agents: List[Dict[str, Any]]) -> Optional[str]:
