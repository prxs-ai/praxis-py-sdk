...
class PlanningResult:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if self.workflow_plan:
            result["workflow_plan"]["created_at"] = (
                self.workflow_plan.created_at.isoformat()
            )
...
class WorkflowPlanner:
    ...
    def __init__(
        ...
        self.llm_client = (
            WorkflowPlannerClient(
                api_key=api_key or "dummy", model="gpt-4", temperature=0.1
            )
            if api_key
            else None
        )
    ...
    async def generate_from_natural_language(
        ...
        logger.info(
            f"Processing planning request {request_id}: {user_request[:100]}..."
        )
        ...
        try:
            ...
            if self.llm_client and network_context.available_tools:
                try:
                    ...
                    workflow_plan = await self._generate_with_llm(
                    ...
                    logger.warning(
                        f"LLM generation failed: {e}, falling back to rule-based approach"
                    )
                    workflow_plan = await self._generate_with_fallback(
                        user_request, network_context
                    )
                    fallback_used = True
            ...
            if (
                planning_request.optimization_goals
                and OptimizationGoal.BALANCE in planning_request.optimization_goals
            ):
                logger.debug("Optimizing workflow plan...")
                try:
                    optimization_result = (
                        await self.plan_optimizer.optimize_workflow_plan(
                            workflow_plan,
                    ...
                    if (
                        optimization_result.estimated_speedup > 1.1
                        ...
                        logger.info(
                            f"Applied optimization with {optimization_result.estimated_speedup:.1f}x speedup"
                        )
            ...
            execution_plan = await self.plan_optimizer.create_execution_plan(
                workflow_plan
            )
            ...
            logger.success(
                f"Generated workflow plan {workflow_plan.id} in {processing_time:.2f}s"
            )
            return result
    ...
    async def execute_workflow_plan(
        self, workflow_plan: WorkflowPlan, execution_plan: ExecutionPlan | None = None
    ) -> dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": str(e), "workflow_id": workflow_plan.id}
        ...
        try:
            ...
            for keyword, tools in keywords_to_tools.items():
                if keyword in partial_lower:
                    available_keyword_tools = [
                        tool for tool in tools if tool in available_tools
                    ]
                    if available_keyword_tools:
                        suggestions.append(
                            f"You can use {', '.join(available_keyword_tools)} for {keyword} operations"
                        )
    ...
    async def _generate_with_llm(
        self, user_request: str, network_context: NetworkContext
    ) -> WorkflowPlan:
        ...
        return await self.llm_client.generate_workflow_from_natural_language(
            user_request=user_request, network_context=network_context
        )
    ...
    async def get_network_status(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                and self.llm_client._initialized,
            }
