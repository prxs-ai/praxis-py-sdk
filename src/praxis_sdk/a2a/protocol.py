...
class A2AProtocolHandler:
            ...
            if isinstance(request_data, str):
                try:
                    ...
                    return create_jsonrpc_error_response(
                        None,
                        create_rpc_error(A2AErrorCode.PARSE_ERROR, f"Invalid JSON: {e}")
                    )
            else:
    ...
    async def _handle_message_send(self, params: Dict[str, Any]) -> Task:
        ...
        try:
            ...
            raise A2AProtocolError(
                ...
                f"Invalid message/send parameters: {e}"
            )
    ...
    async def _handle_tasks_get(self, params: Dict[str, Any]) -> Task:
        ...
        try:
            ...
            if not task:
                raise A2AProtocolError(
                    A2AErrorCode.TASK_NOT_FOUND,
                    f"Task '{get_params.id}' not found"
                )
            ...
            raise A2AProtocolError(
                A2AErrorCode.INVALID_PARAMS,
                f"Invalid tasks/get parameters: {e}"
            )
    ...
    async def _handle_tasks_list(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if list_params.state:
                state_tasks = [t for t in all_tasks if t.status.state == list_params.state]
    ...
    async def _handle_agent_status(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "tasks": {
                "counts_by_state": {state.value: count for state, count in task_counts.items()},
...
class A2AMessageBuilder:
    ...
    @staticmethod
    def create_user_message(text: str, context_id: Optional[str] = None) -> Message:
    ...
    @staticmethod
    def create_message_send_request(message: Message, request_id: Union[str, int]) -> JSONRPCRequest:
        """Create a message/send JSON-RPC request."""
        
        return JSONRPCRequest(
            id=request_id,
            method="message/send",
            params={"message": message.dict()}
        )
    
    @staticmethod
    def create_tasks_get_request(task_id: str, request_id: Union[str, int]) -> JSONRPCRequest:
