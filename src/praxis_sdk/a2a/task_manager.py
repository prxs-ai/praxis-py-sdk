...
class TaskManager:
    ...
    async def update_task_status(
        ...
        async with self._lock:
            ...
            if not task:
                logger.warning(f"[TaskID: {task_id}] Attempted to update non-existent task")
                return False
            ...
            if not self._is_valid_transition(old_state, new_state):
                logger.error(f"[TaskID: {task_id}] Invalid state transition: {old_state} -> {new_state}")
                return False
    ...
    async def add_artifact(
        ...
        async with self._lock:
            task = self._tasks.get(task_id)
            if not task:
                logger.warning(f"[TaskID: {task_id}] Attempted to add artifact to non-existent task")
                return None
            ...
            logger.info(f"[TaskID: {task_id}] Artifact '{name}' added (ID: {artifact.artifact_id})")
    ...
    async def add_message_to_history(
        ...
        async with self._lock:
            task = self._tasks.get(task_id)
            if not task:
                logger.warning(f"[TaskID: {task_id}] Attempted to add message to non-existent task")
                return False
            ...
            if len(task.history) > self.max_history_size:
                ...
                logger.debug(f"[TaskID: {task_id}] Trimmed history to {self.max_history_size} messages")
            
            logger.debug(f"[TaskID: {task_id}] Message '{message.message_id}' added to history")
    ...
    async def cleanup_completed_tasks(self, older_than_hours: int = 24) -> int:
        ...
        async with self._lock:
            ...
            if cleaned_count > 0:
                logger.info(f"Cleaned up {cleaned_count} completed tasks older than {older_than_hours}h")
    ...
    async def check_timeouts(self) -> int:
        ...
        async with self._lock:
            ...
            for task_id in timed_out_tasks:
                ...
                logger.warning(f"[TaskID: {task_id}] Task timed out and marked as failed")
...
async def complete_task_with_text_response(
    ...
    if artifact_name:
        await task_manager.add_artifact(
            ...
            description=f"Task completion response"
        )
...
async def fail_task_with_error(
    ...
    return await task_manager.update_task_status(
        ...
        TaskState.FAILED,
        agent_message,
        error_details or error_message
    )