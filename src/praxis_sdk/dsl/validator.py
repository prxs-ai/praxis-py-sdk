...
class DSLValidator:
    ...
    def validate_ast(
        self, ast: AST, available_tools: list[str] | None = None
    ) -> ValidationResult:
    ...
    def _validate_token_structure(
        self, tokens: list[Token], result: ValidationResult
    ) -> None:
        ...
        for i, token in enumerate(tokens):
            ...
            if (
                token.value.upper() in self.reserved_keywords
                and token.type == TokenType.VALUE
            ):
                result.add_issue(
                    ValidationLevel.WARNING,
    ...
    def _validate_call_token(
        self, token: Token, token_path: str, result: ValidationResult
    ) -> None:
    ...
    def _validate_argument_structure(
        self, args: list[str], args_path: str, result: ValidationResult
    ) -> None:
    ...
    def _validate_command_sequences(
        self, tokens: list[Token], result: ValidationResult
    ) -> None:
    ...
    def _validate_node(
        ...
        for i, child in enumerate(node.children):
            self._validate_node(
                child, f"{node_path}.children[{i}]", result, available_tools
            )
    ...
    def _validate_tool_arguments(
        self, node: ASTNode, node_path: str, result: ValidationResult
    ) -> None:
    ...
    def _validate_execution_dependencies(
        self, ast: AST, result: ValidationResult
    ) -> None:
    ...
    def _validate_performance_concerns(
        self, ast: AST, result: ValidationResult
    ) -> None:
