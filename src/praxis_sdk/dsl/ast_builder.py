...
from .types import (
    Token, TokenType, ASTNode, NodeType, AST, 
...
class ASTBuilder:
    ...
    def _parse_node(self, stream: TokenStream) -> Optional[ASTNode]:
        """
    ...
    def _get_positional_param_name(self, tool_name: str, position: int, remaining_args: List[str]) -> Optional[str]:
        """
    ...
    def _parse_conditional_node(self, stream: TokenStream) -> ASTNode:
        ...
        while stream.has_more():
            ...
            if command == 'ELSE':
                ...
                continue
            elif command in ['ENDIF', 'END_IF']:
                stream.advance()  # Consume terminator
                break
            elif command in ['IF', 'PARALLEL', 'SEQUENCE', 'WORKFLOW']:
                # Nested blocks
                break
        ...
        logger.debug(f"Parsed CONDITIONAL node: condition='{condition}', if_children={len(if_children)}, else_children={len(else_children)}")
