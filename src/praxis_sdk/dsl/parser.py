...
from .types import (
    AST, ASTNode, NodeType, Token, ExecutionContext, ExecutionResult,
...
class AgentInterface:
    ...
    async def find_agent_with_tool(self, tool_name: str) -> Optional[str]:
    ...
    async def execute_remote_tool(
        ...
        raise DSLExecutionError(f"Remote tool execution not available for {tool_name} on {peer_id}")
...
class AdvancedDSLParser:
    ...
    def __init__(self, agent_interface: Optional[AgentInterface] = None):
    ...
    async def execute_ast(
        ...
        try:
            ...
            return ExecutionResult(
                success=False,
                error=str(e),
                metadata={"failed_at": "ast_execution"}
            )
    ...
    async def _execute_tool_with_agent(self, tool_name: str, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if peer_id:
                ...
                result = await self.agent_interface.execute_remote_tool(peer_id, tool_name, args)
    ...
    async def _execute_parallel_node(
        ...
        tasks = [
            self._execute_node(child, context, use_cache) 
        ...
        for i, child in enumerate(node.children):
            try:
                ...
                if not result.get("success", True):
                    self.logger.warning(f"Sequence execution stopped at step {i} due to failure")
                    return {
                        "type": "sequence",
                        "results": results,
                        "status": "failed",
    ...
    def _evaluate_condition(self, condition: str, context: Optional[ExecutionContext] = None) -> bool:
    ...
    async def _execute_workflow_node(self, node: ASTNode, context: Optional[ExecutionContext] = None) -> Dict[str, Any]:
    ...
    async def _execute_task_node(self, node: ASTNode, context: Optional[ExecutionContext] = None) -> Dict[str, Any]:
        """Execute TASK node."""
    ...
    async def _execute_agent_node(self, node: ASTNode, context: Optional[ExecutionContext] = None) -> Dict[str, Any]:
        """Execute AGENT node."""
    ...
    def _get_available_tools(self) -> Optional[List[str]]:
        ...
        if self.agent_interface and hasattr(self.agent_interface.agent, 'get_available_tools'):
    ...
    async def generate_workflow_from_natural_language(
        ...
        if network_context is None:
            ...
            if self.agent_interface:
                ...
                if tools:
                    for tool in tools:
                        network_context.add_tool({"name": tool, "description": f"Tool: {tool}"})
