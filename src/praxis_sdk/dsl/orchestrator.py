...
class DSLOrchestrator:
    ...
    async def execute_command(self, command: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"🤖 ROUTING → LLM Orchestration (no pattern matching)")
    ...
    async def _execute_with_llm_orchestration(self, command: str, context: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        logger.info(f"🔍 P2P DISCOVERY: Scanning network for available agents...")
        ...
        for agent in available_agents:
            ...
            logger.info(f"   👤 {agent.get('name', 'unknown')} ({agent.get('type', 'unknown')}): {agent_tools} tools")
        ...
        logger.info(f"🔧 TOTAL TOOLS AVAILABLE: {len(all_tools)} tools across all agents")
...
"""
        ...
        try:
            ...
            if functions:
                logger.debug(f"   📋 First function structure: {json.dumps(functions[0], indent=2, ensure_ascii=False)}")
            ...
            return await self._process_llm_orchestration_response(response, messages, available_agents, original_command=command)
    ...
    async def execute_workflow(self, dsl_workflow: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        ...
        logger.info(f"Executing DSL workflow with {len(dsl_workflow.splitlines())} commands")
        ...
        try:
            for i, command in enumerate(commands):
                ...
                step_context = {
                    **workflow_context,
                    ...
                    "workflow_id": context.get("workflow_id", f"workflow_{int(asyncio.get_event_loop().time())}")
                }
                ...
                if not result.get("success", False) and not context.get("continue_on_error", False):
                    logger.error(f"Workflow stopped at step {i+1} due to error: {result.get('error', 'Unknown error')}")
                    break
            ...
            workflow_result = {
                ...
                "results": results,
                "execution_time": sum(r["result"].get("execution_time", 0) for r in results),
            ...
            logger.success(f"Workflow completed: {successful_steps}/{total_steps} steps successful")
    ...
    def _get_help(self, topic: Optional[str]) -> Dict[str, Any]:
...
"""
    ...
    async def generate_workflow_from_natural_language(
        ...
        try:
            ...
            workflow_plan = await self.advanced_parser.generate_workflow_from_natural_language(
                user_request, 
                network_context
            )
            
            if workflow_plan:
                logger.info(f"Generated workflow plan: {workflow_plan.id} with {len(workflow_plan.nodes)} nodes")
                ...
                try:
                    ...
                    if not peer_ids:
                        ...
                        try:
                            ...
                            peer_ids = list((getattr(p2p, 'peer_tools', {}) or {}).keys())
        ...
        try:
            if hasattr(self.agent, 'get_available_tools'):
                ...
                return [
                    ...
                    {"name": "python_analyzer", "description": "Execute Python code in container"}
                ]
        except Exception as e:
            logger.warning(f"Failed to get local tools: {e}")
            return []
    
    async def _get_all_available_tools(self, agents: List[Dict[str, Any]], local_tools: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _build_agent_context(self, agents: List[Dict[str, Any]], local_tools: List[Dict[str, Any]]) -> str:
    ...
    def _convert_tools_to_functions(self, tools: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for tool in tools:
            # Debug logging
            logger.debug(f"Converting tool: {tool.get('name', 'unknown')} - parameters type: {type(tool.get('parameters'))}")
            function_def = {
                "type": "function",
                "function": {
    ...
    async def _process_llm_orchestration_response(self, response, messages: List[Dict], agents: List[Dict], original_command: str = "") -> Dict[str, Any]:
        ...
        if message.tool_calls:
            ...
            for i, tool_call in enumerate(message.tool_calls, 1):
                try:
                    logger.info(f"   🧩 ToolCall[{i}]: name={tool_call.function.name} args={tool_call.function.arguments}")
                except Exception:
                    pass
            ...
            for i, tool_call in enumerate(message.tool_calls, 1):
                ...
                logger.info(f"🔧 TOOL EXECUTION {i}/{len(message.tool_calls)}: {tool_name}")
                logger.info(f"   📥 Arguments: {tool_args}")
                
                # Execute tool (local or P2P)
                result = await self._execute_tool_with_p2p(tool_name, tool_args, agents)
                
                logger.info(f"   📤 Result: {result.get('success', 'unknown')} - {result.get('output', result.get('error', 'no output'))[:100]}")
            ...
            try:
                ...
                chose_computer = any(tr["tool"] == "computer_open_app" for tr in tool_results)
                    ...
                    if app:
                        ...
                        for a in agents:
                            ...
                            if selected:
                                break
                        if selected:
                            logger.info(f"🔁 Augmenting with computer_open_app on {selected.get('name')}")
                            cr = await self._execute_p2p_tool("computer_open_app", {"app": app}, selected)
                            tool_results.append({
                ...
                if has_tg_intent and not chose_tg:
                    ...
                    try:
                        if hasattr(self.agent, 'p2p_service') and self.agent.p2p_service:
                            ...
                            logger.info(f"P2P visible peer tools: { {k[:8]: [t.get('name') for t in v] for k, v in visible.items()} }")
                    ...
                    if not selected:
                        for a in agents:
                            if a.get("type") == "p2p":
                                ...
                                break
                    if selected:
                        logger.info(f"🔁 Augmenting with telegram_poster on {selected.get('name')}")
                        # Extract brief message from text as a fallback
                        msg = original_command.strip()
                        cr = await self._execute_p2p_tool("telegram_poster", {"message": msg}, selected)
                        tool_results.append({
                ...
                if any(k in text for k in ["телеграм", "telegram"]):
                    ...
                    for a in agents:
                        ...
                        if selected:
                            break
                    if selected:
                        logger.info(f"Heuristic: selecting agent {selected.get('name')} for telegram_poster")
                        res = await self._execute_p2p_tool("telegram_poster", {"message": original_command.strip()}, selected)
                        return {
    ...
    async def _execute_tool_with_p2p(self, tool_name: str, arguments: Dict[str, Any], agents: List[Dict]) -> Dict[str, Any]:
        ...
        logger.info(f"   🔍 Found {len(candidate_agents)} agents with '{tool_name}' tool")
        ...
        try:
            if agent_type == "local":
                ...
                logger.info(f"🌐 P2P EXECUTION: Running '{tool_name}' on remote agent {agent_name}")
    ...
    async def _execute_local_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self.agent, 'invoke_tool'):
                ...
                logger.info(f"   ✅ LOCAL EXECUTION COMPLETE: {result.get('success', 'unknown')}")
                return result
            else:
                # NO FALLBACK - System must have proper invoke_tool method
                error_msg = f"Agent has no invoke_tool method - cannot execute '{tool_name}'"
    ...
    async def _execute_p2p_tool(self, tool_name: str, arguments: Dict[str, Any], agent: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if agent_name and hasattr(p2p, 'call_remote_tool_by_peer_name'):
                result = await p2p.call_remote_tool_by_peer_name(agent_name, tool_name, arguments)
            else:
    ...
    async def _heuristic_fallback(self, command: str, agents: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for a in agents:
            ...
            if selected:
                break
        if not selected:
            return {"success": False, "error": "computer-use tool not found on any agent"}
        logger.info(f"Heuristic: selecting agent {selected.get('name')} for computer_open_app")
