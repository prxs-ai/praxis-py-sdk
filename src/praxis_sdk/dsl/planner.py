...
from .types import AST, ASTNode, DSLValidationError, ExecutionContext, NodeType
...
class NetworkContext:
    ...
    def get_best_agent_for_tool(self, tool_name: str) -> str | None:
...
class WorkflowPlan:
    ...
    def add_node(self, node_id: str, node_type: str, data: dict[str, Any]) -> None:
        """Add workflow node."""
        self.nodes.append({"id": node_id, "type": node_type, "data": data})

    def add_edge(
        self, edge_id: str, source: str, target: str, edge_type: str = "default"
    ) -> None:
        """Add workflow edge."""
        self.edges.append(
...
"""
    ...
    async def generate_workflow_from_natural_language(
        ...
        try:
            ...
            context_info = self._build_context_prompt(
                network_context, execution_context
            )
            ...
            plan.metadata = {
                ...
                "parallel_opportunities": workflow_data.get(
            ...
            self.logger.info(
                f"Generated workflow plan: {plan.id} with {len(plan.nodes)} nodes"
            )
            return plan
    ...
    def _build_context_prompt(
        ...
        if network_context.available_agents:
            ...
            for agent in network_context.available_agents:
                ...
                agents_info.append(
                    f"- Agent {agent_id}: tools={agent_tools}, load={load:.2f}"
                )
        ...
        if execution_context:
            ...
            if execution_context.agent_capabilities:
                context_parts.append(
                    f"Agent Capabilities: {execution_context.agent_capabilities}"
                )

        return "\n\n".join(context_parts)

    async def _call_llm_for_planning(
        self, user_request: str, context_info: str
    ) -> dict[str, Any]:
    ...
    def _ast_to_workflow_plan(
        self, ast: AST, plan: WorkflowPlan, network_context: NetworkContext
    ) -> None:
        ...
        for ast_node in ast.nodes:
            nodes, edges = self._convert_ast_node(
                ast_node, network_context, node_counter, edge_counter
            )
            ...
            nodes.append(
                {
                    ...
                    "data": {
                        "agent_id": agent_id,
                        "agent_capabilities": network_context.agent_capabilities.get(
                            agent_id, []
                        ),
                        "position": {"x": 100, "y": 100 * node_start_id},
                    },
    ...
    async def validate_plan(
        self, plan: WorkflowPlan, network_context: NetworkContext
    ) -> list[str]:
        ...
        available_tool_names = {
            tool.get("name") for tool in network_context.available_tools
        }
