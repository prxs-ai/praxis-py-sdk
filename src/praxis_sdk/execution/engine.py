...
class DaggerExecutionEngine(ExecutionEngine):
    ...
    async def execute(
        ...
        try:
            ...
            logger.info(f"      🔧 Initializing Dagger connection with log streaming...")
            if not DAGGER_AVAILABLE or dagger is None:
                raise ExecutionError("Dagger is not available - this should have been caught in __init__")
            ...
            async with dagger.connection(config) as conn:
                ...
                logger.info(f"   💾 STEP 5.5: MOUNTING PIP CACHE (optional)")
                if 'python' in spec.image.lower() or any('pip' in cmd for cmd in spec.command if isinstance(cmd, str)):
                    try:
                        ...
                        if pip_cache_dir.exists():
                            logger.info(f"      📦 Mounting pip cache: {pip_cache_dir} → /root/.cache/pip")
                            host_pip_cache = client.host().directory(str(pip_cache_dir))
                            container = container.with_directory('/root/.cache/pip', host_pip_cache)
                            logger.info(f"   ✅ STEP 5.5 SUCCESS: Pip cache mounted")
                        else:
                            logger.info(f"   ✅ STEP 5.5 SKIPPED: No pip cache directory found")
                    except Exception as cache_error:
                        logger.warning(f"   ⚠️  STEP 5.5 WARNING: Could not mount pip cache: {cache_error}")
                else:
                    logger.info(f"   ✅ STEP 5.5 SKIPPED: Not a Python environment")
                ...
                logger.info(f"   📥 STEP 7: RETRIEVING EXECUTION RESULTS WITH LIVE STREAMING")
                logger.info(f"      🔄 Starting container execution...")
                logger.info(f"      📡 === CONTAINER OUTPUT START ===")
                    
                # Get stdout (equivalent to Go: Stdout(ctx)) - MUST be inside connection context
                logger.info(f"      🔄 Executing and streaming output...")
                ...
                try:
                    ...
                    logger.error(f"      ❌ Container execution failed!")
                    stderr_text = ""
                    try:
                ...
                logger.info(f"      📡 === CONTAINER OUTPUT END ===")
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                async with asyncio.timeout(timeout_seconds):
                    logger.info(f"      🔄 Creating Dagger connection (timeout: {timeout_seconds}s)...")
                ...
                logger.warning(f"      ⏰ Connection attempt {attempt + 1} timed out after {timeout_seconds}s")
                
                if attempt < max_retries - 1:
                    logger.info(f"      ⏳ Waiting {delay}s before retry...")
                    await asyncio.sleep(delay)
                else:
                    logger.warning(f"   ❌ All {max_retries} connection attempts timed out")
                    return False
                    
            except Exception as conn_error:
                error_str = str(conn_error)
                logger.warning(f"      ❌ Connection attempt {attempt + 1} failed: {error_str}")
                
                # Check for known GraphQL schema issues
                if "includeDeprecated" in error_str and "Unknown argument" in error_str:
                    logger.warning("      🔧 Known GraphQL schema compatibility issue detected")
                    logger.warning("      🔧 This is a compatibility issue between Dagger versions")
                    
                    try:
                        logger.info("      🔄 Attempting simplified connection...")
                        async with asyncio.timeout(30):  # Shorter timeout for workaround
                            async with dagger.Connection() as client:
                                container = client.container().from_("alpine:latest")
                                logger.info("      ✅ Workaround successful - basic operations work")
                                return True
                        ...
                        if attempt == max_retries - 1:
                            ...
                            logger.error("      - Recommended fix: downgrade gql or upgrade Dagger")
                            logger.error("      - Will fallback to Docker SDK execution engine")
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.warning(f"   ❌ All {max_retries} attempts failed. Last error: {error_str}")
                        return False
