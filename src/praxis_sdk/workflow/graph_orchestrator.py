...
class GraphOrchestrator:
    ...
    def __init__(
        self,
        agent: Agent,
        dsl_orchestrator: DSLOrchestrator | None = None,
    ...
    async def create_from_ast(
        ...
        logger.info(
            f"Creating workflow graph {workflow_id} from {len(ast_nodes)} AST nodes"
        )
        ...
        workflow = await self._build_graph_from_ast(
            ast_nodes, workflow_id, execution_context
        )
        ...
        if validation_issues:
            logger.warning(
                f"Workflow {workflow_id} validation issues: {validation_issues}"
            )

        logger.info(
            f"Created workflow graph {workflow_id} with {len(workflow.nodes)} nodes"
        )
        return workflow

    async def execute_workflow(
        self, workflow: WorkflowGraph, track_progress: bool = True
    ) -> WorkflowResult:
        ...
        try:
            ...
            result = await asyncio.wait_for(
                execution_task, timeout=self.execution_timeout
            )
            ...
            logger.info(
                f"Workflow execution {execution_id} completed in {execution_time:.2f}s"
            )
            return result
    ...
    async def get_execution_status(self, execution_id: str) -> dict[str, Any] | None:
        ...
        completed_nodes = sum(
            1 for node in workflow.nodes.values() if node.status == NodeStatus.COMPLETED
        )
        failed_nodes = sum(
            1 for node in workflow.nodes.values() if node.status == NodeStatus.FAILED
        )
        running_nodes = sum(
            1 for node in workflow.nodes.values() if node.status == NodeStatus.RUNNING
        )
        ...
        return {
            ...
            "started_at": workflow.execution_context.started_at.isoformat()
            if workflow.execution_context.started_at
            else None,
            "node_statuses": {
                node_id: {
    ...
    async def _build_graph_from_ast(
        ...
        current_parallel_group: list[str] | None = None
    ...
    async def _execute_workflow_internal(
        self, workflow: WorkflowGraph, track_progress: bool
    ) -> WorkflowResult:
        ...
        try:
            ...
            while True:
                ...
                ready_nodes = [
                    node for node in ready_nodes if node.id not in running_nodes
                ]
                    ...
                    for task in done:
                        ...
                        if node_result.status == NodeStatus.COMPLETED:
                            ...
                            if await self._is_critical_failure(node, workflow):
                                logger.error(
                                    f"Critical failure in node {node_id}, stopping workflow"
                                )
                                break
                        ...
                        if track_progress:
                            await self._emit_node_completion(
                                node, node_result, workflow
                            )
                ...
                if not running_nodes and not ready_nodes:
                    ...
                    if remaining_nodes > 0:
                        logger.warning(
                            f"Workflow stuck: {remaining_nodes} nodes cannot execute"
                        )
                        result.status = "failed"
                        result.error_summary = (
                            f"{remaining_nodes} nodes blocked by dependencies"
                        )
                    break
    ...
    async def _execute_single_node(
        self, node: WorkflowNode, workflow: WorkflowGraph, track_progress: bool
    ) -> NodeResult:
        ...
        try:
            ...
            result = await self.node_executor.execute_node(
                node, workflow.execution_context
            )
    ...
    async def _process_conditional_node(
        self, ast_node: ASTNode, workflow: WorkflowGraph, node_stack: list[WorkflowNode]
    ):
    ...
    async def _is_critical_failure(
        self, node: WorkflowNode, workflow: WorkflowGraph
    ) -> bool:
    ...
    async def _update_execution_stats(
        self, result: WorkflowResult, execution_time: float
    ):
        ...
        self.execution_stats["average_execution_time"] = (
            prev_avg * (total_execs - 1) + execution_time
        ) / total_execs
    ...
    async def _emit_workflow_completion(
        self, workflow: WorkflowGraph, result: WorkflowResult
    ):
    ...
    async def _emit_node_completion(
        self, node: WorkflowNode, result: NodeResult, workflow: WorkflowGraph
    ):
