...
from .models import ExecutionContext, NodeResult, NodeStatus, WorkflowNode
...
class NodeExecutor:
    ...
    def __init__(self, agent: Agent, dsl_orchestrator: DSLOrchestrator | None = None):
        self.agent = agent
    ...
    async def execute_node(
        ...
        while retry_count <= max_retries:
            try:
                ...
                logger.warning(
                    f"Node {node.id} execution failed (attempt {retry_count + 1}): {e}"
                )
                ...
                if retry_count <= max_retries and self._should_retry_error(e):
                    logger.info(
                        f"Retrying node {node.id} (attempt {retry_count + 1}/{max_retries + 1})"
                    )
    ...
    async def _execute_local_node(
        ...
        if self.dsl_orchestrator:
            ...
            result = await self.dsl_orchestrator.execute_command(
                command=dsl_command, context=context.to_dict()
            )
        else:
    ...
    async def _execute_remote_node(
        ...
        try:
            ...
            if response.get("success"):
                return response
            raise RuntimeError(
                f"Remote execution failed: {response.get('error', 'Unknown error')}"
            )
    ...
    async def _execute_direct_tool_call(
        ...
        result = await self.mcp_service.call_tool(
            tool_name=node.command, arguments=tool_args
        )
    ...
    def _process_execution_result(
        self, raw_result: Any, node: WorkflowNode, context: ExecutionContext
    ) -> dict[str, Any]:
    ...
    async def execute_test_node(
        ...
        test_context = ExecutionContext(
            workflow_id="test_workflow", execution_id="test_execution"
        )
    ...
    async def validate_node_executable(self, node: WorkflowNode) -> list[str]:
        ...
        if node.agent_id and node.agent_id != self.agent.config.agent_id:
            if not self.p2p_service:
                issues.append(
                    "P2P service required for remote execution but not available"
                )
            else:
                ...
                if node.agent_id not in peers:
                    issues.append(
                        f"Remote agent {node.agent_id} not found in peer list"
                    )
