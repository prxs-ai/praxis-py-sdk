...
from praxis_sdk.a2a.models import Task, TaskState, Message, A2AAgentCard
...
class ExecuteDSLRequest(BaseModel):
    ...
    context_id: Optional[str] = Field(None, description="Context ID for command execution")
...
class CreateTaskRequest(BaseModel):
    ...
    message: Message
    priority: Optional[int] = Field(5, ge=1, le=10, description="Task priority (1=highest, 10=lowest)")
...
class TaskQueryParams(BaseModel):
    ...
    state: Optional[TaskState] = Field(None, description="Filter by task state")
    context_id: Optional[str] = Field(None, description="Filter by context ID")
    limit: int = Field(100, ge=1, le=1000, description="Maximum number of tasks to return")
...
class ToolInvokeRequest(BaseModel):
    ...
    context_id: Optional[str] = Field(None, description="Context ID for tool execution")
    timeout: Optional[int] = Field(300, ge=1, le=3600, description="Tool timeout in seconds")
    engine: Optional[ToolExecutionEngine] = Field(None, description="Preferred execution engine")
...
class ToolInfo(BaseModel):
    ...
    engine: ToolExecutionEngine = Field(ToolExecutionEngine.BUILTIN, description="Execution engine")
...
class ToolsListResponse(APIResponse):
    ...
    categories: Optional[Dict[str, List[str]]] = Field(None, description="Tool categories")
...
class ToolInvokeResponse(APIResponse):
    ...
    estimated_duration: Optional[int] = Field(None, description="Estimated execution time")
...
class StatisticsResponse(APIResponse):
    ...
    p2p_service: Optional[Dict[str, Any]] = Field(None, description="P2P service statistics")
...
class ExecuteWorkflowRequest(BaseModel):
    ...
    context_id: Optional[str] = Field(None, description="Execution context ID")
    timeout: Optional[int] = Field(600, ge=1, le=3600, description="Workflow timeout in seconds")
...
class ChatResponse(APIResponse):
    ...
    context_id: Optional[str] = Field(None, description="Chat context ID")
