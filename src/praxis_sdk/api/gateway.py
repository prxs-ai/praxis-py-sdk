...
from praxis_sdk.a2a.models import (
    ...
    Task, TaskState, Message, MessageRole, Part, PartKind, 
...
from praxis_sdk.bus import event_bus, Event, EventType, EventFilter
...
class APIGateway:
    ...
    def _setup_routes(self):
        ...
        async def execute_command(request: Union[ExecuteRequest, JSONRPCRequest]):
            """
        ...
        try:
            ...
            await event_bus.publish_data(
                EventType.DSL_COMMAND_RECEIVED,
                {
                    ...
                    "command": " ".join([part.text or "" for part in message.parts if part.kind == PartKind.TEXT])
                },
                source="api_gateway",
                correlation_id=task.id
            )
    ...
    async def _handle_tasks_list(self, request: JSONRPCRequest) -> JSONRPCResponse:
        ...
        if state:
            try:
                ...
                filtered_tasks = [t for t in filtered_tasks if t.status.state == state_enum]
            except ValueError:
                error = create_rpc_error(
                    A2AErrorCode.INVALID_PARAMS,
    ...
    async def _handle_websocket_connection(self, websocket: WebSocket):
        ...
        event_bus.websocket_manager.add_connection(connection_id, send_channel, event_filter)
