...
class StaticFileServer:
    ...
    async def serve_file(
        self, file_path: str, directory_type: str = "reports"
    ) -> FileResponse:
    ...
    async def upload_file(
        self, file: UploadFile, directory_type: str = "uploads"
    ) -> FileUploadResponse:
        """Handle file upload."""
        try:
            ...
            safe_filename = (
                ...
                if file.filename
                else f"{upload_id}{file_extension}"
            )
            ...
            logger.info(
                f"File uploaded successfully: {safe_filename} ({file_size} bytes)"
            )
    ...
    async def list_files(
        self, directory_type: str = "reports", subdirectory: str = ""
    ) -> FileListResponse:
        ...
        try:
                    ...
                    if file_info:
                        ...
                        if not item.is_dir():
                            ...
                            file_info["downloadUrl"] = (
                                f"/{directory_type}/{relative_path}"
                            )
            ...
            # Sort by name
            files_info.sort(
                key=lambda x: (not x.get("is_directory", False), x.get("name", ""))
            )
    ...
    async def delete_file(
        self, file_path: str, directory_type: str = "uploads"
    ) -> dict[str, Any]:
        """Delete file."""
        try:
            # Determine base directory
    ...
    async def create_sample_files(self):
        ...
        try:
            ...
            sample_report_data = {
                ...
                "data": {"processed_items": 42, "success_rate": 95.5, "errors": 2},
                "metadata": {"agent": "Praxis Python Agent", "version": "1.0.0"},
            }
...
class StaticFileHandlers:
    ...
    async def get_file_info(
        filename: str, directory: str = "reports"
    ) -> dict[str, Any]:
        """Get file information."""
        try:
            # Determine base directory
