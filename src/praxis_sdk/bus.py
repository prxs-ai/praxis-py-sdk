...
class EventFilter:
    ...
    def __init__(
        ...
        source_patterns: Optional[Set[str]] = None,
    ...
    def matches(self, event: Event) -> bool:
        ...
        if self.source_patterns and event.metadata.source:
            if not any(
                pattern in event.metadata.source 
...
class EventBus:
    ...
    def __init__(self, buffer_size: int = 1000, max_concurrent_handlers: int = 100):
        ...
        self._send_channel: Optional[trio.MemorySendChannel] = None
        self._receive_channel: Optional[trio.MemoryReceiveChannel] = None
        
        # Control
        self._running = False
        self._nursery: Optional[trio.Nursery] = None
        self._semaphore: Optional[trio.Semaphore] = None
    ...
    async def start(self, nursery: Optional[trio.Nursery] = None) -> None:
        """Start the event bus."""
        if self._running:
            return
        ...
        self._send_channel, self._receive_channel = trio.open_memory_channel(self.buffer_size)
    ...
    def subscribe(
        ...
        if weak_ref:
            ref = weakref.ref(handler) if hasattr(handler, '__self__') else lambda: handler
    ...
    async def publish(self, event: Event) -> None:
        ...
        if not self._send_channel:
            logger.debug(f"Event published: {event.type.value} from {event.metadata.source}")
            return
