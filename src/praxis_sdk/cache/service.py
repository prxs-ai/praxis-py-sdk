...
class CacheService:
    ...
    def clear(self, category: str | None = None) -> dict[str, Any]:
        """Clear cache entries.
        ...
        try:
            if category:
                ...
                keys_to_delete = [
                    key for key in self.cache.keys() if key.startswith(f"{category}:")
                ]
                ...
                logger.info(
                    f"Cleared cache category '{category}': {deleted_count} items"
                )
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        miss_rate = (
            (self.stats["misses"] / total_requests * 100) if total_requests > 0 else 0.0
        )
        ...
        return {
            "cache_info": {
                ...
                "memory_usage_mb": round(
                    self.stats["memory_usage_bytes"] / (1024 * 1024), 2
                ),
            },
            "operations": {
            ...
            "top_accessed": [
                {"key": key, "access_count": count} for key, count in most_accessed
            ],
        }
    ...
    def get_category_stats(self, category: str) -> dict[str, Any]:
        ...
        total_size = sum(
            entry.get("size_estimate", 0) for entry in category_items.values()
        )
            ...
            items_info.append(
                {
                    ...
                    "expired": entry.get("ttl") and time.time() > entry["ttl"]
                    if entry.get("ttl")
                    else False,
                }
            )
