    ...
    async def send_request(self, request: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            response = await self.client.post(
                self.endpoint, json=request, headers=merged_headers
            )
...
class MCPServer:
    ...
    def __init__(
        ...
        config: MCPConfig,
        event_bus: EventBus,
        external_endpoints: list[str | dict[str, Any]] | None = None,
    ):
        self.config = config
        self.event_bus = event_bus
        self.external_endpoints: list[str | dict[str, Any]] = external_endpoints or []
    ...
    def _setup_system_tools(self):
        ...
        async def get_system_info() -> dict[str, Any]:
            ...
            await self.event_bus.publish_data(
                EventType.LOG_ENTRY, {"tool": "get_system_info", "info": info}
            )
        ...
        async def execute_python(code: str) -> Any:
            ...
            try:
                ...
                await self.event_bus.publish_data(
                    EventType.LOG_ENTRY, {"tool": "execute_python", "status": "success"}
                )
    ...
    async def start(self):
        ...
        combined_eps: list[str | dict[str, Any]] = []
                    ...
                    async with httpx.AsyncClient() as client:
                        ...
                        if response.status_code == 200:
                            ...
                            if info_response.status_code == 200:
                                ...
                                self.external_clients[server_id] = httpx.AsyncClient(
                                    base_url=base
                                )
    ...
    async def _call_mcp(
        ...
        async with client.stream(
            ...
            if response.status_code != 200:
                ...
                raise RuntimeError(
                    f"HTTP {response.status_code}: {body.decode() if body else ''}"
                )
    ...
    async def _connect_to_external_endpoint(self, endpoint: str | dict[str, Any]):
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                try:
                    ...
                    if resp.status_code != 200:
                        logger.debug(
                            f"Endpoint {name} SSE probe returned {resp.status_code}"
                        )
                except Exception as e:
            ...
            async with httpx.AsyncClient(base_url=base) as init_client:
                try:
                    init_response = await self._call_mcp(
                        init_client,
                        {
                            "jsonrpc": "2.0",
                            "method": "initialize",
                    ...
                    logger.debug(
                        f"Initialization response from {name}: {init_response}"
                    )
                    ...
                    self.external_clients[server_id] = httpx.AsyncClient(
                        base_url=base, headers=default_headers
                    )
                    ...
                    logger.warning(
                        f"Failed to initialize external MCP at {base}: {init_err}"
                    )
        except Exception as e:
        ...
        try:
            ...
            if server_config.command and len(server_config.command) > 0:
                ...
                try:
                    ...
                    logger.debug(
                        f"Initialization response from {server_name}: {init_response}"
                    )
                    ...
                    logger.error(
                        f"Failed to initialize MCP server {server_name}: {init_err}"
                    )
    ...
    async def _fetch_server_tools(self, server_id: str):
        ...
        try:
            ...
            logger.info(
                f"Found {len(tools)} tools on {server_id}: {[t['name'] for t in tools]}"
            )
    ...
    async def invoke_tool(
        ...
        if tool_name in self.builtin_tools:
            ...
            logger.info("   🏠 BUILT-IN TOOL: Executing locally")
        ...
        logger.info("   🔍 SEARCHING EXTERNAL TOOLS...")
    ...
    async def _invoke_external_tool(
        ...
        logger.info("   🌐 EXTERNAL TOOL INVOCATION:")
    ...
    async def register_dagger_tool(
        ...
        """
        from ..execution.contracts import EngineType, ToolContract
        ...
        async def dagger_tool_handler(**args) -> str:
            ...
            logger.info(
                f"   📂 Working Dir: {contract.engine_spec.get('working_dir', '/workspace')}"
            )
            logger.info(f"   💾 Input Arguments: {args}")
            logger.info("   🔄 DELEGATING TO EXECUTION ENGINE...")
            ...
            if result.success:
                ...
                logger.info("   🎉 DAGGER TOOL COMPLETED SUCCESSFULLY")
        ...
        self.tool_metadata[name] = {
            ...
            "parameters": {
                ...
                "required": list(
                    self._infer_dagger_parameters(name, command, description).keys()
                ),
            },
        }
    ...
    async def register_local_tool(
        ...
        async def local_tool_handler(**args) -> str:
            ...
            if result.success:
                ...
                logger.info(
                    f"   📤 Output: {(result.output or 'Tool executed successfully')[:200]}..."
                )
                return result.output or "Tool executed successfully"
    ...
    async def setup_default_dagger_tools(
        self, agent=None, shared_path: str = "/app/shared"
    ):
...
""",
    ...
    def _infer_dagger_parameters(
        self, name: str, command: list[str], description: str = None
    ) -> dict[str, Any]:
        ...
        if name == "text_processor":
            return {"text": {"type": "string", "description": "Text to process"}}

        # Default parameters for unknown tools
        return {"input": {"type": "string", "description": f"Input for {name}"}}
