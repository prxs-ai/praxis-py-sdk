...
from praxis_sdk.mcp.server import HTTPTransport, MCPTransport, SSETransport
...
class MCPClient:
    ...
    async def _create_transport(self, config: MCPServerConfig) -> MCPTransport:
        ...
        if config.transport_type == "subprocess":
    ...
    async def _initialize_server(self, server_name: str, transport: MCPTransport):
        ...
        response = await transport.send_request(
            "initialize",
            {
                ...
                "clientInfo": {"name": "praxis-mcp-client", "version": "1.0.0"},
            },
        )
        ...
        logger.debug(
            f"Initialized MCP server {server_name}: {response.get('result', {})}"
        )
    ...
    async def _discover_tools(self, server_name: str, transport: MCPTransport):
        ...
        if "error" in response:
            logger.error(
                f"Failed to discover tools from {server_name}: {response['error']}"
            )
            return
    ...
    async def _schedule_reconnection(self, server_name: str):
        ...
        for attempt in range(server_config.max_retries):
            ...
            logger.info(
                f"Reconnection attempt {attempt + 1}/{server_config.max_retries} for {server_name}"
            )
            ...
            try:
                ...
                logger.error(
                    f"Reconnection attempt {attempt + 1} failed for {server_name}: {e}"
                )
    ...
    async def refresh_tools(self, server_name: str):
        ...
        if server_name in self.transports and self.connection_status.get(
            server_name, False
        ):
            transport = self.transports[server_name]
            await self._discover_tools(server_name, transport)
            ...
            logger.warning(
                f"Cannot refresh tools for disconnected server: {server_name}"
            )
