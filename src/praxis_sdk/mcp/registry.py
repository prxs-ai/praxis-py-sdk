...
class ToolRegistry:
        ...
        if "properties" in schema and "required" in schema:
            ...
            for prop in required_props:
                if prop not in arguments:
                    logger.warning(
                        f"Missing required argument '{prop}' for tool '{tool_name}'"
                    )
                    return False
    ...
    async def _execute_external_tool(
        ...
        try:
            ...
            logger.error(
                f"External tool execution failed: {original_name} on {server_url} - {str(e)}"
            )
            raise
    ...
    def search_tools(self, query: str, category: str | None = None) -> list[ToolInfo]:
