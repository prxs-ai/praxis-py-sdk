...
class MCPIntegration:
    ...
    def _create_server_config(
        self, endpoint: str | dict[str, Any], index: int
    ) -> MCPServerConfig:
        ...
        for server_name, tools in available_tools.items():
            for tool in tools:
                ...
                if hasattr(self.mcp_server, "external_tools"):
                    server_tools = self.mcp_server.external_tools.setdefault(
                        server_name, {}
                    )
    ...
    def _setup_event_handlers(self):
        ...
        event_bus.subscribe(
            "mcp.external_server_connected", self._on_external_server_connected
        )
        event_bus.subscribe(
            "mcp.external_server_disconnected", self._on_external_server_disconnected
        )
    ...
    async def _on_peer_disconnected(self, event_type: str, data: dict[str, Any]):
        ...
        if peer_id:
            ...
            if peer_id in self.peer_tools:
                ...
                logger.info(
                    f"Removed {len(tools)} tools from disconnected peer {peer_id}"
                )
    ...
    async def _on_p2p_tool_request(self, event_type: str, data: dict[str, Any]):
        ...
        try:
            ...
            if self.p2p_service and peer_id:
                await self.p2p_service.send_tool_response(
                    peer_id, {"tool_name": tool_name, "result": result, "success": True}
                )
            ...
            if self.p2p_service and peer_id:
                await self.p2p_service.send_tool_response(
                    peer_id, {"tool_name": tool_name, "error": str(e), "success": False}
                )
    ...
    async def _on_external_server_connected(
    ...
    async def _on_external_server_disconnected(
        self, event_type: str, data: dict[str, Any]
    ):
        ...
        if self.mcp_server:
            tools_to_remove = [
                name
                for name in self.mcp_server.tool_registry.tools.keys()
                if name.startswith("external_")
                and self.mcp_server.tool_registry.tools[name].server_url == server_name
            ]
