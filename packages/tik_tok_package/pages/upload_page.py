...
class UploadPage:
    ...
    def toggle_autor_rules(self):
        ...
        autor_rule_button = self.driver.find_element(
            By.XPATH, "//div[1]/div[6]/div/div/div/div[2]/div"
        )
        autor_rule_button.click()
        for _ in range(60):
            try:
                self.driver.find_element(
                    By.CSS_SELECTOR, "div.jsx-478649263.tool-tip.success"
                )
    ...
    def click_post_button(self):
        ...
        post_button.click()
        log.info(
            "Completed video upload, check manually. If the video is not uploaded, check the error message in the console. If the video is uploaded, you can close the browser."
        )
        # except Exception as e:
        #     log.error(f"Error while upload video: {e}")
