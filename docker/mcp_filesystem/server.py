...
class WriteFileParams(BaseModel):
    ...
    mode: Optional[str] = Field(default="overwrite", description="Write mode: overwrite, append, create_only")
...
class ReadFileParams(BaseModel):
    ...
    encoding: Optional[str] = Field(default="utf-8", description="File encoding")
    max_size: Optional[int] = Field(default=10485760, description="Maximum file size to read (bytes)")

class ListDirectoryParams(BaseModel):
    path: Optional[str] = Field(default=".", description="Directory path relative to shared directory")
    include_hidden: Optional[bool] = Field(default=False, description="Include hidden files and directories")
    recursive: Optional[bool] = Field(default=False, description="List files recursively")

class CreateDirectoryParams(BaseModel):
    path: str = Field(description="Directory path relative to shared directory")
    create_parents: Optional[bool] = Field(default=True, description="Create parent directories if they don't exist")
        ...
        self.tools = {
            "write_file": {
                ...
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "filename": {"type": "string", "description": "Target file path relative to shared directory"},
            ...
            "read_file": {
                ...
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "filename": {"type": "string", "description": "Source file path relative to shared directory"},
                        "encoding": {"type": "string", "default": "utf-8"},
            ...
            "list_directory": {
                ...
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "path": {"type": "string", "default": ".", "description": "Directory path relative to shared directory"},
            ...
            "create_directory": {
                ...
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "path": {"type": "string", "description": "Directory path relative to shared directory"},
    ...
    async def write_file(self, params: WriteFileParams) -> Dict[str, Any]:
        ...
        try:
            ...
            if params.mode == "create_only" and file_path.exists():
                raise ValueError(f"File '{params.filename}' already exists and mode is create_only")
    ...
    async def read_file(self, params: ReadFileParams) -> Dict[str, Any]:
        ...
        try:
            ...
            if file_size > params.max_size:
                raise ValueError(f"File size ({file_size} bytes) exceeds maximum allowed size ({params.max_size} bytes)")
    ...
    try:
        ...
        if method == "tools/list":
            ...
            if tool_name == "write_file":
                ...
                result = await server.list_directory(ListDirectoryParams(**tool_arguments))
            elif tool_name == "create_directory":
                result = await server.create_directory(CreateDirectoryParams(**tool_arguments))
            else:
                return MCPResponse(
                    id=request.id,
