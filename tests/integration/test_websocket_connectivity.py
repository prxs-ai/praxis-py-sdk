...
async def test_websocket_connection(wait_for_services, orchestrator_ws, test_helper):
    ...
    await test_helper.send_websocket_message(orchestrator_ws, "ping", ping_message["payload"])
    
    # Try to receive a response
    try:
        response = await test_helper.receive_websocket_message(orchestrator_ws, timeout=10.0)
...
async def test_websocket_dsl_command_streaming(wait_for_services, orchestrator_ws, test_helper):
    ...
    for i in range(max_events):
        try:
            ...
            print(f"Received event {i+1}: {event['type'] if 'type' in event else 'unknown'}")
...
@pytest.mark.websocket
@pytest.mark.asyncio
async def test_websocket_workflow_execution(wait_for_services, orchestrator_ws, test_helper):
    ...
    await test_helper.send_websocket_message(
        orchestrator_ws,
        "EXECUTE_WORKFLOW", 
        workflow_message["payload"]
    )
    
    # Monitor workflow events
    workflow_events = []
    expected_events = {"workflowStart", "nodeStatusUpdate", "workflowComplete", "workflowError"}
...
async def test_websocket_concurrent_connections(wait_for_services):
    ...
    try:
        ...
        print(f"Received responses from {successful_responses}/{len(connections)} connections")
...
@pytest.mark.websocket
@pytest.mark.asyncio
async def test_websocket_large_message_handling(wait_for_services, orchestrator_ws, test_helper):
...
async def test_websocket_heartbeat_and_keepalive(wait_for_services, orchestrator_ws, test_helper):
    ...
    for i in range(message_count):
        try:
            ...
            print(f"Heartbeat response {received_responses}: {response.get('type', 'unknown')}")
...
@pytest.mark.websocket
@pytest.mark.asyncio
async def test_websocket_message_ordering(wait_for_services, orchestrator_ws, test_helper):
