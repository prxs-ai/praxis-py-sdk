...
@pytest.mark.slow
@pytest.mark.asyncio
async def test_dagger_python_execution(wait_for_services, orchestrator_client, test_helper, setup_test_files):
...
'''
    ...
    try:
        response = await test_helper.send_dsl_command(orchestrator_client, dagger_command)
        ...
        if response.get("success") or "result" in response:
            ...
            result_file = setup_test_files["script"].parent / "dagger_analysis_result.json"
            if result_file.exists():
                ...
                print("Dagger analysis result file not found - execution may have failed")
        else:
...
@pytest.mark.slow
@pytest.mark.asyncio
async def test_dagger_container_isolation(wait_for_services, orchestrator_client, test_helper, shared_dir):
...
'''
    ...
    try:
        response = await test_helper.send_dsl_command(orchestrator_client, isolation_command)
        ...
        if response.get("success") or "result" in response:
            ...
            if result_file.exists():
                ...
                assert isolation_results["shared_dir_access"] == True  # Should have shared access
...
async def test_tool_error_handling(wait_for_services, orchestrator_client, test_helper):
    ...
    response = await test_helper.send_dsl_command(orchestrator_client, invalid_tool_command)
...
@pytest.mark.slow
@pytest.mark.asyncio
async def test_concurrent_tool_execution(wait_for_services, orchestrator_client, test_helper, shared_dir):
    ...
    print(f"Successfully executed {successful_commands}/{len(concurrent_commands)} concurrent commands")
...
async def test_mcp_error_handling(wait_for_services, mcp_client):
    ...
    invalid_file_request = {
        ...
        "params": {
            "name": "read_file",
            "arguments": {
                "filename": "/invalid/path/file.txt"
            }
        }
    }
