            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt == max_retries - 1:
                        logger.error(f"Service {name} not ready after {max_retries} attempts: {e}")
...
class TestHelper:
    ...
    @staticmethod
    async def send_dsl_command(client: httpx.AsyncClient, command: str) -> Dict[str, Any]:
    ...
    @staticmethod
    async def send_a2a_message(client: httpx.AsyncClient, message_text: str) -> Dict[str, Any]:
    ...
    @staticmethod
    async def send_websocket_message(websocket, message_type: str, payload: Dict[str, Any]) -> None:
    ...
    @staticmethod
    async def receive_websocket_message(websocket, timeout: float = 10.0) -> Dict[str, Any]:
