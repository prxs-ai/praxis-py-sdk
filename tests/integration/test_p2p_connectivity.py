...
@pytest.mark.p2p
@pytest.mark.asyncio
async def test_peer_discovery(wait_for_services, orchestrator_client, worker_fs_client, worker_analytics_client):
    ...
    if response.status_code == 200:
        ...
        analytics_peer_names = [card.get("name", "") for card in analytics_peers.values()]
...
@pytest.mark.p2p
@pytest.mark.asyncio
async def test_agent_card_structure(wait_for_services, orchestrator_client, worker_fs_client, worker_analytics_client):
...
@pytest.mark.slow
@pytest.mark.asyncio
async def test_p2p_tool_invocation(wait_for_services, orchestrator_client, test_helper, shared_dir):
    ...
    try:
        ...
        if response.get("success") or "result" in response:
            ...
            if test_file.exists():
                ...
                print("P2P tool invocation: file not found, may indicate delegation issue")
        else:
...
async def test_p2p_capability_advertisement(wait_for_services, orchestrator_client):
    ...
    expected_orchestrator_caps = {
        "orchestration", "task_delegation", "workflow_management", "llm_integration"
    }
    ...
    if response.status_code == 200:
        ...
        for peer_id, card in cards.items():
            ...
            if "filesystem" in peer_name.lower():
                ...
                if found_caps:
                    print(f"✓ Filesystem worker has expected capabilities: {found_caps}")
...
async def test_mdns_service_registration(wait_for_services, orchestrator_client):
    ...
    if len(cards) > 0:
        ...
        print("mDNS discovery: no peers found (may be expected in isolated test environment)")
...
async def test_p2p_protocol_versioning(wait_for_services, orchestrator_client):
    ...
    if response.status_code == 200:
        ...
        for peer_id, card in cards.items():
            ...
            print(f"Peer {card.get('name', 'unknown')} protocol version: {peer_version}")
