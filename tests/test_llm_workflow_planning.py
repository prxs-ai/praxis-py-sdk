...
from praxis_sdk.llm.context_builder import NetworkContextBuilder, AgentInfo, ToolInfo
...
class TestWorkflowPlanner:
    ...
    def mock_p2p_service(self):
        """Mock P2P service."""
        service = Mock()
        service.get_all_agent_cards = AsyncMock(return_value={
            "agent1": {
                "id": "test-agent",
                "capabilities": {
    ...
    def mock_mcp_registry(self):
        ...
        registry.discover_all_tools = AsyncMock(return_value={
            "filesystem": [
                {"name": "file_read", "description": "Read files", "inputSchema": {"properties": {}}},
                {"name": "file_write", "description": "Write files", "inputSchema": {"properties": {}}}
            ]
        })
        return registry
    ...
    async def test_network_status(self, workflow_planner):
        ...
        with patch.object(workflow_planner.context_builder, 'get_network_health') as mock_health, \
             patch.object(workflow_planner.context_builder, 'build_network_context') as mock_context:
...
class TestWorkflowPlanOptimizer:
    ...
    def mock_context_builder(self):
        ...
        builder = Mock()
        builder.build_network_context = AsyncMock(return_value=NetworkContext(
            available_agents=[],
...
class TestIntegrationScenarios:
    ...
    async def test_end_to_end_workflow_planning(self, full_setup):
        ...
        result = await planner.generate_from_natural_language(
            ...
            optimization_goals=[OptimizationGoal.BALANCE]
        )
