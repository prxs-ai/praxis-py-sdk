...
def config():
    return P2PConfig(
        ...
        relay_addr="/ip4/127.0.0.1/tcp/8888/p2p/QmTest",
        noise_key="test_noise_key"
    )
...
class TestLibP2PNode:
    ...
    def test_init_host(self, node):
        with patch('base_agent.p2p.libp2p.node.new_host') as mock_new_host, \
    ...
    async def test_initialize(self, node):
        ...
        with patch.object(node, '_init_host', return_value=mock_host), \
             ...
             patch('base_agent.p2p.libp2p.node.handle_card') as mock_handle_card:
    ...
    def test_init_host_with_noise_protocol(self, node):
        with patch('base_agent.p2p.libp2p.node.new_host') as mock_new_host, \
            ...
            mock_noise.assert_called_once_with(
                libp2p_keypair=node.keypair,
                noise_privkey="decoded_noise_key"
            )
            mock_new_host.assert_called_once()
            assert result == mock_host
