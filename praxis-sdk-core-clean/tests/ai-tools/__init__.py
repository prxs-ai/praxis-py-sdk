...
def mock_settings():
    class MockSettings:
        ...
        create_comment_to_message_prompt = "Create a reply for {0} comment about {1}: {2}"
...
async def test_check_answer_is_needed_true(mocker):
    mocker.patch("send_openai_request.check_answer_is_needed_prompt", "Check if {0} comment needs a reply")
    mocker.patch("send_openai_request.send_openai_request", AsyncMock(return_value="True"))
...
async def test_check_answer_is_needed_false(mocker):
    mocker.patch("send_openai_request.check_answer_is_needed_prompt", "Check if {0} comment needs a reply")
    mocker.patch("send_openai_request.send_openai_request", AsyncMock(return_value="False"))
    mocker.patch.object(logger, "info")
...
async def test_create_comment_to_message(mocker):
    mocker.patch("send_openai_request.create_comment_to_message_prompt", "Create a reply for {0} comment about {1}")
    mocker.patch("send_openai_request.send_openai_request", AsyncMock(return_value="Generated reply"))
    mocker.patch.object(logger, "info")
    ...
    send_openai_request.assert_called_once_with(
        messages=[
            {"role": "system", "content": f"Create a reply for TWITTER comment about {PROJECT_DESCRIPTION}"},
