...
from twitter_ambassador_utils.config import cipher, get_hvac_client, get_settings
...
class TwitterAuthClient:
    ...
    async def callback(cls, state: str, code: str):
        ...
        combined_data["access_token"] = cipher.encrypt(
            tokens_data["access_token"].encode()
        ).decode()
        combined_data["refresh_token"] = cipher.encrypt(
            tokens_data["refresh_token"].encode()
        ).decode()
    ...
    def _store_session_data_in_vault(cls, session_id: str, data: dict) -> None:
        ...
        try:
            ...
            cls._store_session_data_in_redis(
                session_id, data, ex=3600
            )  # 1 hour expiration
    ...
    def get_session_data_from_vault(cls, session_id: str) -> dict | None:
        ...
        try:
            response = cls.HVAC_CLIENT.secrets.kv.v2.read_secret_version(
                path=cls.SESSION_PATH.format(session_id=session_id), mount_point="dev/"
            )
    ...
    def get_session_data(cls, session_id: str) -> dict | None:
        ...
        if not data:
            logger.warning(
                f"Session data not found in Vault, trying Redis for session ID: {session_id}"
            )
    ...
    async def get_twitter_data(cls, username: str) -> dict:
        ...
        try:
            resp = cls.HVAC_CLIENT.secrets.kv.v2.read_secret_version(
                path=cls.AUTH_PATH.format(user_id=username), mount_point="dev/"
            )
            logger.info(f"Retrieved Twitter data from Vault for user ID: {username}")
            return resp["data"]["data"]
        except Exception as e:
            logger.error(
                f"Failed to get Twitter data from Vault for user {username}: {e}"
            )
            return {}
    ...
    async def get_access_token(cls, username: str) -> str:
        ...
        try:
            ...
            if current_time + 300 > expires_at:
                logger.info(
                    f"Token expired or expiring soon for user {username}, refreshing..."
                )
                new_tokens = await cls._refresh_tokens(
                    twitter_data.get("refresh_token")
                )
                if new_tokens:
                    ...
                    if current_time < expires_at:
                        access_token = cipher.decrypt(
                            twitter_data["access_token"].encode()
                        ).decode()
                    else:
