...
def parse_tweet_from_twitter_api(
    tweet_data: dict, users: dict, referenced_tweets: dict = None
) -> Tweet:
        ...
        if ref_type == "retweeted":
            if ref_id in referenced_tweets:
                ...
                retweeted_status = RetweetedStatus(
                    ...
                    user=rt_user,
                    retweet_count=rt_data.get("public_metrics", {}).get(
                        "retweet_count", 0
                    ),
                    favorite_count=rt_data.get("public_metrics", {}).get(
                        "like_count", 0
                    ),
                )
            ...
            if ref_id in referenced_tweets:
                ...
                quoted_status = QuotedStatus(
                    ...
                    user=qt_user,
                    retweet_count=qt_data.get("public_metrics", {}).get(
                        "retweet_count", 0
                    ),
                    favorite_count=qt_data.get("public_metrics", {}).get(
                        "like_count", 0
                    ),
                )
    ...
    return Tweet(
        ...
        in_reply_to_status_id_str=tweet_data.get("in_reply_to_status_id"),
    )
...
async def fetch_user_tweets(access_token: str, username: str) -> list[Tweet]:
    ...
    users_data = {
        user["id"]: user for user in result.get("includes", {}).get("users", [])
    }
    ...
    for tweet_data in tweets_data:
        try:
            tweet = parse_tweet_from_twitter_api(
                tweet_data, users_data, referenced_tweets
            )
            tweets.append(tweet)
        except Exception as e:
    ...
    while tweet_id:
        try:
            reply_tweet = await get_tweet_by_link(
                access_token, f"https://twitter.com/i/status/{tweet_id}"
            )
