...
from tweetscout_utils import (
    ...
    fetch_user_tweets,
    get_conversation_from_tweet,
    get_tweet_by_link,
    parse_quoted_status,
    parse_retweeted_status,
    parse_tweet,
    parse_user,
    search_tweets,
)
...
async def test_make_request_client_error(mock_settings, mocker):
    ...
    mocker.patch(
        ...
        side_effect=aiohttp.ClientResponseError(
            status=400, request_info=None, history=None, message="Bad Request"
        ),
    )
    ...
    mocker.patch(
        ...
        side_effect=[
            aiohttp.ClientResponseError(
                status=429, request_info=None, history=None, message="Rate Limit"
            )
        ]
        * 2
        + [AsyncMock(status=200, json=AsyncMock(return_value={"data": "test"}))],
    )
...
async def test_fetch_user_tweets(mock_settings, mocker, sample_tweet_data):
    mocker.patch("tweetscout_utils.get_settings", return_value=mock_settings)
    mocker.patch(
        "tweetscout_utils._make_request",
        AsyncMock(return_value={"tweets": [sample_tweet_data]}),
    )
    ...
    tweetscout_utils._make_request.assert_called_once_with(
        "https://api.tweetscout.io/v2/user-tweets", {"link": "https://x.com/testuser"}
    )
...
async def test_search_tweets(mock_settings, mocker, sample_tweet_data):
    ...
    assert tweets[0].id_str == "456"
    tweetscout_utils._make_request.assert_called_once_with(
        "https://api.tweetscout.io/v2/search-tweets", {"query": "query"}
    )
...
async def test_get_tweet_by_link(mock_settings, mocker, sample_tweet_data):
    mocker.patch("tweetscout_utils.get_settings", return_value=mock_settings)
    mocker.patch(
        "tweetscout_utils._make_request", AsyncMock(return_value=sample_tweet_data)
    )
...
async def test_get_conversation_from_tweet(mock_settings, mocker, sample_tweet_data):
    ...
    mocker.patch("tweetscout_utils.get_settings", return_value=mock_settings)
    mocker.patch(
        "tweetscout_utils._make_request", AsyncMock(return_value=reply_tweet_data)
    )
